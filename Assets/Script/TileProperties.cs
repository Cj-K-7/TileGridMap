...
public class TileProperties : MonoBehaviour
{
    ...
    void GetNeighbours()
    {
        ...
        if (tileX > 0 && Neighbour(tileX - 1, tileY).GetComponent<TileProperties>().checker == 0)
        {
            beforeX = Neighbour(tileX - 1, tileY);
        }
        if (tileX < map.mapSizeX - 1 && Neighbour(tileX + 1, tileY).GetComponent<TileProperties>().checker == 0)
        {
            afterX = Neighbour(tileX + 1, tileY);
        }
        if (tileY > 0 && Neighbour(tileX, tileY - 1).GetComponent<TileProperties>().checker == 0)
        {
            beforeY = Neighbour(tileX, tileY - 1);
        }
        if (tileY < map.mapSizeY - 1 && Neighbour(tileX, tileY + 1).GetComponent<TileProperties>().checker == 0)
        {
